...
├── requirements.txt
├── data/
│   └── asistentes.csv (puede estar vacío o no existir aún)
...
def show_program():
    ...
    with tabs[1]:
        st.markdown("""
        - **09:00** Talleres simultáneos (interpretación, patrimonio, guionismo)
        - **13:00** Comida
        - **15:00** Actividad de integración cultural""")

    with tabs[2]:
        st.markdown("""
        - **08:00** Salidas de campo a Chiapa de Corzo y Cañón del Sumidero
        - **14:00** Clausura y entrega de reconocimientos""")
...
def check_in():
    ...
    if qr_code:
        ...
        if qr_code in df['QR_ID'].values:
            ...
            st.error("❌ QR no reconocido. Verifica el código ingresado.")
